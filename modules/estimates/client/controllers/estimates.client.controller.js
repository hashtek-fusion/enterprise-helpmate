    ...
    function ($scope, $stateParams, $location, Authentication, Estimates, EstimatesSvc, ConfigSvc, $modal, $window) {
        ...
        $scope.create = function () {
            ...
            estimate.$save(function (response) {
                ...
                if(roles.indexOf('editor')!==-1 && $scope.estType==='MDE' && estimate.estimates.estimateValid === 'NO' && estimate.estimates.originalComplexity.key!==estimate.estimates.complexity.key) $scope.sendMail(response.pmtId, 'MDE_ESTIMATE');
        ...
        $scope.update = function () {
            ...
            estimate.$update(function () {
                ...
                var roles= $scope.authentication.user.roles;
                if(roles.indexOf('editor')!==-1 && $scope.estType==='MDE' && estimate.estimates.estimateValid === 'NO' && estimate.estimates.originalComplexity.key!==estimate.estimates.complexity.key) $scope.sendMail(estimate.pmtId, 'MDE_ESTIMATE');
            }, function (errorResponse) {
                $scope.error = errorResponse.data.message;
            });
        };
